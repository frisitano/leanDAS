...
# Self-referential defaults (compile-time constants for the "self" case).
...
# Maximum sizes for Array allocations (used when parameters are runtime).
# These are the self-referential values since self is always the largest circuit.
MAX_BYTECODE_CLAIM_SIZE_PADDED = BYTECODE_CLAIM_SIZE_PADDED
MAX_INNER_PUBLIC_MEMORY_LOG_SIZE = INNER_PUBLIC_MEMORY_LOG_SIZE


def fs_observe_dynamic(fs, data, n_chunks):
    """Observe n_chunks digest-sized blocks into Fiat-Shamir state (runtime n_chunks)."""
    result: Mut = Array(9)
...
def copy_many_ef_dynamic(a, b, n):
    """Copy n extension field elements from a to b (runtime n)."""
    for i in range(0, n):
        copy_5(a + i * DIM, b + i * DIM)
    return


def zero_pad_claim_dynamic(claim, bytecode_claim_size, max_padded):
    """Zero-pad bytecode claim from bytecode_claim_size to max_padded (runtime sizes)."""
    for k in range(bytecode_claim_size, max_padded):
        claim[k] = 0
    return
...
def recursion_parameterized(inner_public_memory, proof_transcript, bytecode_value_hint,
                            log_guest_bl, inner_pis, inner_pm_log):
    """Verify a STARK proof with runtime-parameterized guest bytecode size.

    Args:
        inner_public_memory: pointer to inner proof's public memory
        proof_transcript: pointer to proof transcript
        bytecode_value_hint: hint for bytecode MLE evaluation
        log_guest_bl: log2(guest bytecode length) — runtime parameter
        inner_pis: public input size of the inner proof — runtime parameter
        inner_pm_log: log2(inner public memory size) — runtime parameter
    """
    # Compute derived values from runtime parameters.
    bytecode_point_n_vars = log_guest_bl + log2_ceil(N_INSTRUCTION_COLUMNS)
    bytecode_claim_size = (bytecode_point_n_vars + 1) * DIM
    bytecode_hash_offset = inner_pis - DIGEST_LEN

    fs: Mut = fs_new(proof_transcript)

    inner_pub_input = inner_public_memory + NONRESERVED_PROGRAM_INPUT_START
    # Observe public input — runtime-sized.
    n_pub_chunks = inner_pis / DIGEST_LEN
    fs = fs_observe_dynamic(fs, inner_pub_input, n_pub_chunks)
    # Observe bytecode hash.
    fs = fs_observe(fs, inner_pub_input + bytecode_hash_offset, DIGEST_LEN)
    ...
    log_bytecode_padded = maximum(log_guest_bl, log_n_cycles)
    ...
    assert MIN_LOG_MEMORY_SIZE <= log_memory
    assert log_memory <= MAX_LOG_MEMORY_SIZE
    assert log_guest_bl <= log_memory
    ...
    # Runtime: use log_guest_bl instead of LOG_GUEST_BYTECODE_LEN
    bytecode_and_acc_point = point_gkr + (n_vars_logup_gkr - log_guest_bl) * DIM
    bytecode_multilinear_location_prefix = multilinear_location_prefix(
        offset / two_exp(log_guest_bl), n_vars_logup_gkr - log_guest_bl, point_gkr
    )
    bytecode_padded_multilinear_location_prefix = multilinear_location_prefix(
        offset / two_exp(log_bytecode_padded), n_vars_logup_gkr - log_bytecode_padded, point_gkr
    )
    # Build padded claim data: [point | value | zero padding]
    # Allocate at max size, use only bytecode_claim_size elements.
    bytecode_claim = Array(MAX_BYTECODE_CLAIM_SIZE_PADDED)
    copy_many_ef_dynamic(bytecode_and_acc_point, bytecode_claim, log_guest_bl)
    copy_many_ef(
        logup_alphas + (log2_ceil(MAX_BUS_WIDTH) - log2_ceil(N_INSTRUCTION_COLUMNS)) * DIM,
        bytecode_claim + log_guest_bl * DIM,
        log2_ceil(N_INSTRUCTION_COLUMNS),
    )
    copy_5(bytecode_value_hint, bytecode_claim + bytecode_point_n_vars * DIM)
    zero_pad_claim_dynamic(bytecode_claim, bytecode_claim_size, MAX_BYTECODE_CLAIM_SIZE_PADDED)
    bytecode_value = bytecode_claim + bytecode_point_n_vars * DIM
    bytecode_value_corrected: Mut = bytecode_value
    for i in unroll(0, log2_ceil(MAX_BUS_WIDTH) - log2_ceil(N_INSTRUCTION_COLUMNS)):
        bytecode_value_corrected = mul_extension_ret(bytecode_value_corrected, one_minus_self_extension_ret(logup_alphas + i * DIM))
    ...
    bytecode_index_value = mle_of_01234567_etc(bytecode_and_acc_point, log_guest_bl)
    retrieved_denominators_value = add_extension_ret(
        retrieved_denominators_value,
        mul_extension_ret(
            bytecode_multilinear_location_prefix,
    ...
    # Dispatch based on table height ordering (sorted by descending height)
    if maximum(table_log_heights[1], table_log_heights[2]) == table_log_heights[1]:
        continue_recursion_ordered_param(
            1, 2, fs, offset,
            retrieved_numerators_value, retrieved_denominators_value,
            table_heights, table_log_heights, point_gkr, n_vars_logup_gkr,
            logup_alphas_eq_poly, logup_c, numerators_value, denominators_value,
            log_memory, inner_public_memory, stacked_n_vars,
            whir_log_inv_rate, whir_base_root, whir_base_ood_points,
            whir_base_ood_evals, num_ood_at_commitment,
            log_n_cycles, log_bytecode_padded, bytecode_and_acc_point,
            value_memory, value_acc, value_bytecode_acc,
            log_guest_bl, inner_pm_log,
        )
    else:
        continue_recursion_ordered_param(
            2, 1, fs, offset,
...
@inline
def continue_recursion_ordered_param(
    second_table,
    third_table,
    fs,
    offset,
    ...
    # Runtime: use inner_pm_log instead of INNER_PUBLIC_MEMORY_LOG_SIZE
    fs, public_memory_random_point = fs_sample_many_ef(fs, inner_pm_log)
    poly_eq_public_mem = poly_eq_extension_dynamic(public_memory_random_point, inner_pm_log)
    public_memory_eval = Array(DIM)
    dot_product_be_dynamic(inner_public_memory, poly_eq_public_mem, public_memory_eval, two_exp(inner_pm_log))
    ...
    # Runtime: use inner_pm_log instead of INNER_PUBLIC_MEMORY_LOG_SIZE
    eq_pub_mem = eq_mle_extension(
        folding_randomness_global + (stacked_n_vars - inner_pm_log) * DIM,
        public_memory_random_point,
        inner_pm_log,
    )
    prefix_pub_mem = multilinear_location_prefix(0, stacked_n_vars - inner_pm_log, folding_randomness_global)
    s = add_extension_ret(
        s,
        mul_extension_ret(mul_extension_ret(curr_randomness, prefix_pub_mem), eq_pub_mem),
    )
    ...
    # Runtime: use log_guest_bl instead of LOG_GUEST_BYTECODE_LEN
    eq_bytecode_acc = eq_mle_extension(
        folding_randomness_global + (stacked_n_vars - log_guest_bl) * DIM,
        bytecode_and_acc_point,
        log_guest_bl,
    )
    prefix_bytecode_acc = multilinear_location_prefix(
        offset / two_exp(log_guest_bl),
        stacked_n_vars - log_guest_bl,
        folding_randomness_global,
    )
    s = add_extension_ret(
        s,
